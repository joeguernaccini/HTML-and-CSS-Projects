...
//this function will roll for random number twice, one for each player
// and determine wich player won the roll
function rollForTurn() {
    ...
    for (i =0; i < xArray.length; i++) {
        ...
        var pOne = xArray[0];
        var pTwo = xArray[1];
        if (pOne == pTwo) { // rigging roll on tie to avoid bug in code. 
            pOne = 1;
            pTwo = 2;
        }
        ...
        setTimeout(function() {writeMsg(txt1); }, 1000); //time delay   !!!! Curly braces???? 0:18 of video
    }
...
function startGame() {
    ...
    btnDisabled(btn); //disable the state button since the game is now afoot
    var btn = document.getElementById('btnStop');
    stopEnabled(btn); //enable the stop button since the game is now afoot

    //assign the active player to the console
    var showPlayer = document.getElementById('showPlayer') //*****Why no ";"? 15:16
...
function btnDisabled(btn) {
    btn.style.color = "fff"; //****** Why no # ?? Why double quotes instead of single ??? */
...
function stopEnabled(btn) {
    btn.style.color = "#fff"; //****** Why double quotes instead of single ??? */
...
//when the user indicated, stop the current game and reset game
function stopGame() {
    ...
    var showPlayer = document.getElementById('showPlayer') // ****** WHY NO ; ??? 20;22
    showPlayer.innerHTML = "Game Stopped";
    showPlayer.style.color='red';

    //reset all sqares to their starting empty state
...
//this function return' the currently assigned avatar for each player
function getAvatars() {
...
// (line 213) as squares are selected they check in with this function to see if that particular
//square has already been assigned and if it had not, record new square with the assigned avatar
...
function checkForWinCon() {
    var squareArray = [];
    var target = document.getElementById('boardState');
    var info = target.innerHTML; //raw array with squared and avatars
...
function check4Tie() {
    ...
    boardState = boardState.split(','); //seperate the sring by commas into an array
    ...
    if (boardState.length >= 9 && check != "That's three in a row, Player 1 wins!" && check != "That's three in a row, Player 2 wins!") {
        ...
        tieSound(); //play sound when a tie has been detected
        writeMsg(txt1);
...
function glowBoard(pos) {
    ...
    for (var i=0;i<squares.length;i++){
        if (i == index0) {
            ...
            var bg3 = squares[i];
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (122, 244, 66)'; }, 100);
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (197, 244, 66)'; }, 200);
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (244, 179, 66)'; }, 300); 
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (66, 244, 235)'; }, 400);
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (122, 244, 66)'; }, 500);
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (122, 244, 66)'; }, 600);
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (197, 244, 66)'; }, 700);
            setTimeout(function() {bg3.style.backgroundcolor = 'rgb (244, 179, 66)'; }, 800);
...
//(line 338) these functions will profuce game sounds depending on the occasion
...
function blink() {
    ...
    setTimeout(function () {body.style.backgroundColor = '#94ced7'; }, 200);
...
function square2Animate() {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        var verdict = recordMoves(square);
        if (verdict == undefined) { //if verdict is empty than the square is unoccupied.
            var paintAvatar = determineAvatar(); //get the corrext avatar to paint for the active player
            ...
            // build new array adding the newly selected square and the asigned avatar
            var currentMove = ","+square+paintAvatar;
            recordMove(currentMove);
            checkForWinCon(); // call function to check if current move completes a winning condition
            avatarPlaced(square,paintAvatar); // end current turn and pass the turn to the other player
...
function square9Animate() {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        //check of the proposed square is valid
        var verdict = recordMoves(square);
        if (verdict == undefined) { //if verdict is empty than the square is unoccupied.
            var paintAvatar = determineAvatar(); //get the corrext avatar to paint for the active player
